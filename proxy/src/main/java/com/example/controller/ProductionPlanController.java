 ...
 * 同时转发产品绿色/商业分析（product-context-demo），纯 LLM 调用，绕过 8003。
 */
@RestController
public class ProductionPlanController {
    ...
    /**
     * 产品绿色/商业分析：转发至 8001，纯 LLM，绕过 8003。
     */
    ...
    /**
     * 生成最优生产方案（JSON）。production-plan-bypass-8001=true 或 8001 返回 405 时，直接返回本地参考方案。
     */
    ...
    private Map<String, Object> buildFallbackOptimalPlan(Map<String, Object> body) {
        ...
        plan.put("summary", "参考方案：涵盖 " + product + " 的完整生产流程，设备 " + equipment.size() + " 台、物料 " + materials.size() + " 种、工序 " + procedures.size() + " 道、工艺路线 " + routes.size() + " 条。");
    ...
    /**
     * 导出 Excel（返回文件流）。production-plan-bypass-8001=true 或 8001 不可用时返回简易本地 Excel。
     */
    ...
    public ResponseEntity<byte[]> exportPdf(@RequestBody Map<String, Object> body) {
        if (cvConfig.isProductionPlanBypass8001()) {
            String msg = "PDF 导出暂不可用。完整 PDF 请设置 cv.production-plan-bypass-8001=false 并启动 8001 服务。";
    ...
    private byte[] buildFallbackExcel(Map<String, Object> body) {
        ...
        try (Workbook wb = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            r2.createCell(1).setCellValue("参考方案，完整导出请启动 8001 服务。");
            int row = 4;
